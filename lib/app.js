    ...
    var FusionApp = (function() {
        ...
        FusionApp.prototype._resolvePlugin = function resolvePlugin(basePath, pluginName) {
            ...
            try {
                ...
                var pluginMetadata = {};
                var pluginRawMetadata = require(pluginMetadataPath);
                pluginMetadata.pluginVersion = pluginRawMetadata.version;
                pluginRawMetadata = pluginRawMetadata['fusionio'] || {};
                pluginMetadata.pluginPath = pluginPath;
                pluginMetadata.pluginImports = [];
                pluginMetadata.pluginExports = [];

                // Check if the plugin provides a version number, and if not, warn about it!
                if(!pluginMetadata.pluginVersion) {
                    console.log('[FusionIO] Warning: Plugin \'' + pluginName + '\' does not provide a version number');
                    pluginMetadata.pluginVersion = 'N/A';
                }
        ...
        FusionApp.prototype._startFusion = function startFusion() {
            ...
            try {
                plugin.pluginFactory(plugin, (function(imports) {
                    return function(serviceName) {
                        if(imports.hasOwnProperty(serviceName)) return imports[serviceName];
                        throw new Error('Required service is missing: ' + serviceName);
                    };
                })(imports), registerExports, {
                    fetchPluginList: function() {
                        var pluginList = [];
                        for(var key in self._config) {
                            var plugin = self._config[key];
                            pluginList.push({ name: plugin.pluginName, version: plugin.pluginVersion });
                        }
                        return pluginList;
                    }
                });
